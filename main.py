...
def build_prompt(meta: dict, shownotes: str, transcript: str) -> str:
    ...
    return f"""你是一个播客笔记整理助手。请根据以下信息，生成一份结构化的 Markdown 笔记。

---
**播客信息**
- 播客名称：{meta['podcast_title']}
- 单集标题：{meta['title']}
- 发布日期：{meta['pub_date']}
...
**转录文本（节选）**
{transcript[:8000] if transcript else '（无转录）'}
---

请输出以下结构的 Markdown 笔记（YAML front-matter + 正文）：

1. YAML front-matter（包含播客、单集、发布日期、时长、链接、整理日期）
2. 一句话总结（本集核心主旨）
3. 主要内容（3-6 个要点，每个要点 2-3 句话）
4. 金句摘录（3-5 句印象深刻的话，标注大致时间戳或来源）
5. 延伸思考（1-3 个值得深入探讨的问题）

严格基于提供的内容，不要脑补主讲人没有说过的内容。"""
